			 ...
			 GAsyncResult * res, gpointer user_data)
{
  ...
  if(node_id == -1 )
  {
	printf("there is resource free to schedule application\n");
	exit(0);
  }
  ...
  if(get_numa_node)
   {
      FILE *fp =  fopen("/tmp/numa_api_result.txt", "w+");
...
main (int argc, char *argv[])
{
 ...
 form_query ("test_algo","get_free_numa_node", cpu, memory);
